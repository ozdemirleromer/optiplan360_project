...
def _has_main_with_list_name(pid: int, list_name: str) -> bool:
    wanted = f"[{_normalize_ui_text(list_name)}]"
...
def run_stage1(config: UiConfig) -> None:
    ...
    # If name already exists or save fails, OptiPlanning shows a warning dialog.
    warning = _find_warning_dialog(pid)
    if warning is not None:
        warn_lines = _read_dialog_message_lines(warning)
        _close_dialog(warning)
        # Some builds can show a transient warning while the target job is already opened.
    ...
    wl_dialog_raw = _wait_visible_dialog(pid, ".opdx", timeout_sec=config.timeout_sec)
