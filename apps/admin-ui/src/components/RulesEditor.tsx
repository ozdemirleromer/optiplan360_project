...
interface RulesEditorProps extends RulesConfig {
  onSave: (data: RulesConfig) => Promise<void>;
  status: string;
}
...
export function RulesEditor({ cmToMmMultiplier, retryCountMax, optiModeDefault, onSave, status }: RulesEditorProps) {
  ...
  useEffect(() => {
    setCm(cmToMmMultiplier);
    setRetry(retryCountMax);
    setMode(optiModeDefault);
  }, [cmToMmMultiplier, optiModeDefault, retryCountMax]);
