...
# models.Base.metadata.create_all kaldÄ±rÄ±ldÄ± –” migration geçmiÅŸini eziyordu.
...
app = FastAPI(
    ...
    description="OptiPlan 360 - Ãœretim Yönetim Sistemi",
...
# Güvenlik header'larÄ± security.py'deki middleware tarafÄ±ndan ekleniyor.




# Burada tekrar eklenmiyordu –” çift kayÄ±t kaldÄ±rÄ±ldÄ±.
...
def _seed_canonical_stations(db: Session) -> None:
    ...
    MASTER_HANDOFF Â§0.6 –” 5 kanonik üretim istasyonunu oluÅŸturur.
    ...
    canonical = [
...

        {
            ...
            "description": "Malzeme hazÄ±rlama ve kesim hazÄ±rlÄ±ÄŸÄ± (Ä°ki okutmalÄ±: 1. okutma baÅŸlangÄ±ç)",
...

        {
            ...
            "description": "ParçalarÄ±n boyutlandÄ±rÄ±lmasÄ± (Ä°ki okutmalÄ±: 2. okutma tamamlandÄ±)",
...

        {
            ...
            "description": "Kalite kontrolü (Ä°ki okutmalÄ±: 1. okutma teslimata hazÄ±r)",
...
def _seed_admin_user(db: Session) -> None:
    ...
    if existing:
        ...
        if not existing.password_hash:
            ...
            logger.info("Admin kullanÄ±cÄ±sÄ±nÄ±n ÅŸifresi güncellendi.")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # Sadece baÄŸlantÄ±yÄ± açÄ±k tutmak ve ping-pong için
...
def health_check(db: Session = Depends(get_db), response: Response = None):
    ...
    API saÄŸlÄ±k kontrolü endpoint'i.
    ...
    Monitoring araçlarÄ± (Prometheus, k8s probe, Uptime Robot) HTTP kodu okur.
...
# config_router.py'e taÅŸÄ±ndÄ± –” app/routers/config_router.py
