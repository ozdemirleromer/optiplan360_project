...
OptiPlan 360 ile Biesse OptiPlanning sistemleri arasındaki veri akışını yönetir
...
class BiesseIntegrationService:
    ...
    def export_materials_to_biesse(
        ...
        OptiPlan 360 malzeme stoklarını Biesse formatında export eder
        ...
        try:
            ...
            for material in materials:
                ...
                if total_stock > 0:
                    ...
                    # Drop elementi (artık malzemeler)
            ...
            return {
                "success": True,
                "message": f"{len(materials)} malzeme Biesse formatında export edildi",
            ...
            logger.error(f"Biesse export hatası: {str(e)}")
    ...
    def import_cutting_plan_from_biesse(self, opjx_file_path: str) -> Dict[str, Any]:
        ...
        Biesse kesim planını OptiPlan 360'a import eder
        ...
        try:
            ...
            if not os.path.exists(opjx_file_path):
                ...
                raise ValidationError("OPJX dosyası bulunamadı")

            # OPJX dosyasını parse et
            ...
            return {
                "success": True,
                "message": f"Kesim planı import edildi: {len(production_records)} kayıt",
            ...
            logger.error(f"Biesse import hatası: {str(e)}")

            raise BusinessRuleError(f"Kesim planı import edilemedi: {str(e)}")
    ...
    def sync_materials_bidirectional(self) -> Dict[str, Any]:
        ...
        İki yönlü malzeme senkronizasyonu
        ...
        try:
            ...
            # Biesse → OptiPlan 360 (varsa kesim planları)
            ...
            for opjx_file in self._get_pending_opjx_files():
                ...
                try:
                    ...
                    logger.warning(f"OPJX import hatası: {opjx_file} - {str(e)}")
            ...
            logger.error(f"İki yönlü senkronizasyon hatası: {str(e)}")

            raise BusinessRuleError(f"Senkronizasyon başarısız: {str(e)}")

    # Yardımcı metodlar

    def _calculate_material_stock(self, db, stock_code: str) -> float:
        """Malzeme stok miktarını hesaplar (stock_code bazli)."""
    ...
    def _get_material_drops(self, db, material_id: int) -> List[Dict[str, Any]]:
        """Malzeme artıklarını getir"""

        # Bu metot stok artıklarını hesaplar

        return []  # Şimdilik boş

    def _parse_opjx_file(self, file_path: str) -> Dict[str, Any]:
        """OPJX dosyasını parse eder"""

        # Basit implementasyon - gerçek OPJX formatına göre güncellenmeli
...
"""
    ...
    def _run_optiplanning(self, opjx_file: Path) -> bool:
        ...
        try:
            ...
            # OptiPlanning'i subprocess ile çalıştır

            result = subprocess.run(
                [str(exe_path), "-f", str(opjx_file), "-a"],  # Otomatik çalıştır
                capture_output=True,
                text=True,
                timeout=300,
            )
            ...
            logger.error(f"OptiPlanning çalıştırma hatası: {str(e)}")
    ...
    def _get_pending_opjx_files(self) -> List[str]:
        """İşlenmeyi bekleyen OPJX dosyalarını bulur"""

        try:

            # Basit implementasyon - gerçek mantığa göre güncellenmeli
