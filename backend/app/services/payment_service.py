...
def update_payment_promise_status(
    ...
    if notes:
        promise.notes = (promise.notes or "") + f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M')}] {notes}"
...
def list_payment_promises(
    ...
    if today_only:
        ...
        query = query.filter(
            and_(
                PaymentPromise.status == "PENDING",
                func.date(PaymentPromise.promise_date) == today
            )
        )
...
def get_payment_statistics(db: Session, account_id: Optional[str] = None) -> Dict:
    ...
    ).with_entities(func.sum(Invoice.remaining_amount)).scalar() or 0
    
    # Ödeme sözü istatistikleri
    pending_promises = db.query(PaymentPromise).filter(
        PaymentPromise.status == "PENDING"
    )
    if account_id:
    ...
    pending_promises_amount = pending_promises.with_entities(
        func.sum(PaymentPromise.promised_amount)
    ).scalar() or 0
    ...
    overdue_promises = pending_promises.filter(
        PaymentPromise.promise_date < datetime.now()
    ).count()
