...
class StockCardService(BaseService[StockCard]):
        ...
        try:
            ...
            for stock in stocks:
                try:
                    ...
                    # Existential kontrol
                    existing = self.db.query(StockCard).filter(
                        StockCard.stock_code == stok_kod
                    ).first()
    ...
    def search_stocks(self, search_text: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            query = self.db.query(StockCard).filter(
    ...
    def get_stock_card(self, stock_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            card = self.db.query(StockCard).filter(
                StockCard.stock_code == stock_code
            ).first()
    ...
    def sync_stock_movements(self, stock_code: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            for move in movements:
                try:
                    # Duplicate kontrolü
                    existing = self.db.query(StockMovement).filter(
                        StockMovement.id == move.get("HAREKET_ID")
                    ).first()
    ...
    def get_stock_list(self, skip: int = 0, limit: int = 50, 
    ...
    def get_low_stock_items(self, threshold_quantity: float = 10) -> List[Dict[str, Any]]:
        ...
        try:
            cards = self.db.query(StockCard).filter(
                StockCard.available_quantity < threshold_quantity,
                StockCard.is_active == True
            ).all()
    ...
    def get_stock_summary_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_quantity = self.db.query(StockCard).filter(
                StockCard.is_active == True
            ).first()
            
            active_cards = self.db.query(StockCard).filter(
                StockCard.is_active == True
            ).count()
            
            recent_movements = self.db.query(StockMovement).order_by(
