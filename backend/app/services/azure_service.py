...
from app.models import AzureConfig, AuditLog
...
class AzureOCRConfig(BaseModel):
    ...
    endpoint: str = Field(..., description="Azure Endpoint URL (örn: https://<name>.cognitiveservices.azure.com)")
...
class AzureService:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 202:
                    ...
                    return {"success": False, "error": f"API Hatası: {response.status_code} - {response.text}"}
    ...
    async def upload_to_blob(self, file_data: bytes, file_name: str, content_type: str = "application/octet-stream") -> AzureBlobUploadResult:
        """
        ...
        if not config:
            return AzureBlobUploadResult(
                success=False,
                error="Azure Blob yapılandırması bulunamadı"
            )
    ...
    def update_config(self, ocr_config: Optional[AzureOCRConfig] = None, 
                     blob_config: Optional[AzureBlobConfig] = None) -> AzureConfigOut:
