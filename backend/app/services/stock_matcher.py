...
class StockMatcher:
    ...
    def suggest_alternatives(
        ...
        thickness_alternatives = list(set(
            m.thickness for m in matches if m.thickness and m.thickness != thickness
        ))
        
        # Alternatif renkler
        color_alternatives = list(set(
            m.color for m in matches if m.color
        ))
...
def quick_search(stock_list: List[Dict], query: str, thickness: Optional[str] = None) -> List[StockMatch]:
