...
def analyze_bottlenecks(db: Session) -> dict:
    ...
    """
    try:
        active_orders = db.query(func.count(Order.id)).filter(Order.status.in_(["NEW", "IN_PRODUCTION"])).scalar() or 0
        ready_orders = db.query(func.count(Order.id)).filter(Order.status == "READY").scalar() or 0
        
        active_stations = db.query(func.count(Station.id)).filter(Station.active == True).scalar() or 0
        ...
        average_capacity = sum(STATION_CAPACITIES.values()) / len(STATION_CAPACITIES) if STATION_CAPACITIES else 100
        ...
        overview_facts = [
            ...
            {"label": "Yapay Zeka Yük Durumu", "value": "AŞIRI YÜKLÜ" if current_load_ratio > 1.4 else "OPTİMAL"}
        ]
