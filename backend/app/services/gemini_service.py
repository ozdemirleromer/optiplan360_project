...
class GeminiService:
    ...
    def __init__(self, api_key: Optional[str] = None):
            ...
            try:
                ...
                if os.path.exists(config_path):
                    with open(config_path, 'r', encoding='utf-8') as f:
                        config = json.load(f)
                        if config.get("current_provider") == "gemini" and config.get("providers", {}).get("gemini"):
        ...
        if not api_key:
            raise ValueError("Gemini API anahtarı gereklidir. AI Konfigürasyon ekranından ayarlayın veya GEMINI_API_KEY environment değişkenini tanımlayın.")
