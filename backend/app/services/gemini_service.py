...
def test_gemini_connection(api_key: str, model: str = "gemini-2.0-flash") -> Dict[str, Any]:
    client = build_gemini_client(api_key)
    try:
        response = client.models.generate_content(model=model, contents="Bağlantı testi")
        return {
            "success": True,
