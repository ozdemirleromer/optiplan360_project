...
class IntegrationSettingsService(BaseService[IntegrationSettings]):
    ...
    def get_settings(self, integration_type: IntegrationTypeEnum, category: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if setting:
                return {
                    ...
                    "settings": json.loads(setting.settings) if isinstance(setting.settings, str) else setting.settings,
    ...
    def update_settings(
        ...
        try:
            ...
            if existing:
                ...
                self.db.commit()
                logger.info(f"Yeni ayar oluşturuldu: {integration_type.value}/{category or 'GLOBAL'}")
    ...
    def toggle_active(self, integration_type: IntegrationTypeEnum, category: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if setting:
                ...
                self.db.commit()
                logger.info(f"Entegrasyon {'aktif' if setting.is_active else 'pasif'}: {integration_type.value}/{category or 'GLOBAL'}")
    ...
    def is_integration_active(self, integration_type: IntegrationTypeEnum, category: Optional[str] = None) -> bool:
    ...
    def is_auto_sync_enabled(self, integration_type: IntegrationTypeEnum, category: Optional[str] = None) -> bool:
