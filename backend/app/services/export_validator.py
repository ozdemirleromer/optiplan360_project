...
class ExportValidator:
        ...
        for measure in measures:
            ...
            if isinstance(length, int) or isinstance(width, int):
                ...
                self.warnings.append(
                    f"Tam sayı değer: {measure}. Ondalığa çevrilecek (Kural #5)"
                )
        ...
        for group in groups:
            if group.get("type") == "ARKALIK":
                ...
                for measure in measures:
                    ...
                    if any(band_edges):
                        self.errors.append(
                            f"Arkalıkta bant var! Satır: {measure} (Kural #9)"
                        )
                        return False
    ...
    def generate_checklist(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "item": "Format .xlsx",
                "status": self._check_format(data),
                "rule": "#3"
            },
            {
                "item": "Excel tag'leri bütün",
                "status": self._verify_excel_tags(data),
                "rule": "#4"
            },
            {
