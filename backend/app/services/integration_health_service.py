...
from ..models import (
    IntegrationSettings, IntegrationSyncJob, IntegrationError,
    IntegrationTypeEnum, SyncStatusEnum
)
...
class IntegrationHealthService:
    ...
    def check_cargo_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "HEALTHY",
                "connection": True,
                "message": "Kargo entegrasyonu aktif"
            }
    ...
    def get_sync_statistics(self, integration_type: IntegrationTypeEnum, hours: int = 24) -> Dict[str, Any]:
        """Senkronizasyon istatistiklerini getir"""
        try:
            since = datetime.utcnow() - timedelta(hours=hours)
            
            # Toplam job sayısı
            total = self.db.query(IntegrationSyncJob).filter(
                IntegrationSyncJob.created_at >= since
            ).count()
            
            # Başarılı job sayısı
            success = self.db.query(IntegrationSyncJob).filter(
                IntegrationSyncJob.created_at >= since,
                IntegrationSyncJob.status == SyncStatusEnum.SUCCESS
            ).count()
            
            # Başarısız job sayısı
            failed = self.db.query(IntegrationSyncJob).filter(
                IntegrationSyncJob.created_at >= since,
                IntegrationSyncJob.status == SyncStatusEnum.FAILED
            ).count()
            
            # Bekleyen job sayısı
            pending = self.db.query(IntegrationSyncJob).filter(
                IntegrationSyncJob.status.in_([SyncStatusEnum.QUEUED, SyncStatusEnum.RUNNING])
            ).count()
            
            # Son başarılı sync
            last_sync_job = self.db.query(IntegrationSyncJob).filter(
                IntegrationSyncJob.status == SyncStatusEnum.SUCCESS
            ).order_by(IntegrationSyncJob.completed_at.desc()).first()
    ...
    def get_recent_errors(self, hours: int = 24, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors = self.db.query(IntegrationError).filter(
                IntegrationError.created_at >= since,
                IntegrationError.is_resolved == False
            ).order_by(IntegrationError.created_at.desc()).limit(limit).all()
