...
class DropOptimizationService:
    ...
    def generate_optimization_report(optimized_parts: List[dict]) -> dict:
        ...
        return {
            ...
            "average_length_gain_per_part": total_length_gain / optimized_count if optimized_count > 0 else 0,
            "average_width_gain_per_part": total_width_gain / optimized_count if optimized_count > 0 else 0,
...
def _band_ticks_match(p1: PartLike, p2: PartLike) -> bool:
    return (
        p1.u1 == p2.u1 and p1.u2 == p2.u2
        and p1.k1 == p2.k1 and p1.k2 == p2.k2
    )
...
class MergeService:
    ...
    def compute_suggestions(
        ...
        if part_group_filter is not None and parts and hasattr(parts[0], "part_group"):
            filtered_with_idx = [(row_start + i, p) for i, p in enumerate(parts) if getattr(p, "part_group", None) == part_group_filter]
        else:
    ...
    def apply_merge_groups(
        ...
        for group in merge_groups:
            ...
            first_row = min(group)
            indices_in_parts = [r - row_start for r in group if row_start <= r < row_start + len(parts)]
            if indices_in_parts:
        ...
        for row_one in sorted(group_by_first.keys()):
            ...
            result.append({
                ...
                "u1": p0.u1, "u2": p0.u2, "k1": p0.k1, "k2": p0.k2,
