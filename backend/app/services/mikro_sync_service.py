...
from ..models import (
    ...
    IntegrationInbox, IntegrationError, SyncStatusEnum, SyncDirectionEnum
)
...
class MikroSyncService:
    ...
    def sync_account_to_mikro(self, account_id: str, account_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            # Mapping kontrolü
            mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="ACCOUNT",
            ...
            # Mikro data hazırla
            mikro_data = {
                "CARI_KOD": account_data.get("mikro_cari_kod") or mapping.external_id if mapping else None,
            ...
            if mapping:
                ...
                if success:
                    ...
                    self.db.commit()
                    return {"success": True, "mikro_cari_kod": mapping.external_id, "message": "Güncellendi"}
            else:
                # CREATE
    ...
    def sync_account_from_mikro(self, cari_kod: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if mapping:
                ...
                mapping.last_synced_at = datetime.utcnow()
                self.db.commit()
                return {"success": True, "account_id": mapping.internal_id, "message": "Güncellendi"}
            else:
    ...
    def sync_invoice_to_mikro(self, invoice_id: str, invoice_data: Dict[str, Any], invoice_lines: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            # Account mapping
            account_mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="ACCOUNT",
                internal_id=invoice_data.get("account_id")
            ).first()
            ...
            # Invoice mapping
            inv_mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="INVOICE",
                internal_id=invoice_id
            ).first()
            
            mikro_data = {
                "FATURA_NO": invoice_data.get("mikro_fatura_no") or inv_mapping.external_id if inv_mapping else f"FTR-{datetime.now().strftime('%Y%m%d%H%M%S')}",
            ...
            self.db.commit()
            return {"success": True, "mikro_fatura_no": fatura_no, "lines_synced": len(invoice_lines)}
    ...
    def sync_quote_to_mikro(self, quote_id: str, quote_data: Dict[str, Any], quote_lines: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            account_mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="ACCOUNT",
                internal_id=quote_data.get("account_id")
            ).first()
            
            if not account_mapping:
                return {"success": False, "error": "Cari hesap Mikro'ya senkronize edilmemiş"}
            
            quote_mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="QUOTE",
                internal_id=quote_id
            ).first()
            
            mikro_data = {
                "TEKLIF_NO": quote_data.get("mikro_teklif_no") or quote_mapping.external_id if quote_mapping else f"TKL-{datetime.now().strftime('%Y%m%d%H%M%S')}",
    ...
    def sync_order_to_mikro(self, order_id: str, order_data: Dict[str, Any], order_items: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            order_mapping = self.db.query(IntegrationEntityMap).filter_by(
                entity_type="ORDER",
                internal_id=order_id
            ).first()
            
            mikro_data = {
                "SIPARIS_NO": order_data.get("mikro_siparis_no") or order_mapping.external_id if order_mapping else f"SIP-{datetime.now().strftime('%Y%m%d%H%M%S')}",
            ...
            self.db.commit()
            return {"success": True, "mikro_siparis_no": siparis_no, "lines_synced": len(order_items)}
