...
# ─── Modül düzeyi saf yardımcı fonksiyonlar ──────────────────────────────────

def _ensure_non_negative(value: Any, field_name: str) -> float:
    """Değerin sayısal ve negatif olmadığını doğrula; float döndür."""
    from app.exceptions import ValidationError
    try:
        numeric = float(value)  # type: ignore[arg-type]
    except (TypeError, ValueError):
        raise ValidationError(f"'{field_name}' alanı sayısal olmalıdır")
    if numeric < 0:
        raise ValidationError(f"'{field_name}' alanı negatif olamaz")
    return numeric


def _ensure_percentage(value: Any, field_name: str) -> float:
    """0-100 arasında yüzde değeri doğrula."""
    from app.exceptions import ValidationError
    v = _ensure_non_negative(value, field_name)
    if v > 100:
        raise ValidationError(f"'{field_name}' alanı 100'ü geçemez")
    return v


def _normalize_phone(value: _Opt[str]) -> _Opt[str]:
    """Telefon numarasını normalleştir (sadece rakam, + öneki korunur)."""
    from app.exceptions import ValidationError
    if not value:
        return None
    raw = str(value).strip()
    ...
    if not digits:
        raise ValidationError("Telefon formatı geçersiz")
    return ("+" if has_plus else "") + digits


def _normalize_tax_id(value: _Opt[str]) -> _Opt[str]:
    """Vergi kimlik numarasını normalleştir (10 veya 11 hane)."""
    from app.exceptions import ValidationError
    if not value:
        return None
    digits = re.sub(r"[^\d]", "", str(value))
    if len(digits) not in (10, 11):
        raise ValidationError("Vergi kimlik numarası 10 veya 11 haneli olmalıdır")
    return digits


def _normalize_account_type(value: _Opt[str]) -> _Opt[str]:
    """Hesap tipini normalleştir (CORPORATE / INDIVIDUAL)."""
    from app.exceptions import ValidationError
    if not value:
        return None
    ...
    valid = {"CORPORATE", "INDIVIDUAL"}
    if upper not in valid:
        raise ValidationError(f"Hesap tipi geçersiz: {value}")
    return upper


def _normalize_dealer_type(value: _Opt[str]) -> _Opt[str]:
    """Bayi tipini normalleştir."""
    from app.exceptions import ValidationError
    if not value:
        return None
    upper = str(value).strip().upper()
    valid = {"BAYII", "DISTRIBÜTÖR", "ACENTE", "BAYI", "DISTRIBUTOR", "AGENT"}
    if upper not in valid:
        raise ValidationError(f"Bayi tipi geçersiz: {value}")
    return upper


def _normalize_tag_list(value: _Opt[str]) -> _Opt[str]:
    """Virgül veya noktalı virgül ile ayrılmış etiket listesini normalleştir."""
...
def create_quote(db: Session, data: dict, lines: list, user_id: int) -> CRMQuote:
    ...
    resolved_quote_number = document_no or _generate_quote_number(db)
    resolved_title = data.get("title") or document_no or resolved_quote_number
    ...
    quote = CRMQuote(
        ...
        quote_number=resolved_quote_number,
        ...
        **{
            k: v
            for k, v in data.items()
            if k not in {"lines", "document_no", "quote_number", "title"}
        },
    ...
    for idx, line_data in enumerate(lines, 1):
        ...
        line_tax_rate = line_data.get("tax_rate", 0) or 0
        line_tax = line_net * (line_tax_rate / 100)
...
def revise_quote(db: Session, quote_id: str, user_id: int) -> Optional[CRMQuote]:
    ...
    new_data = {
        ...
        "document_no": old_quote.quote_number,
