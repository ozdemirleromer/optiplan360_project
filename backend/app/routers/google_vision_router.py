...
def get_google_vision_stats(
    ...
    # Vision OCR'ları say
    total_jobs = (
        db.query(func.count(OCRJob.id)).filter(OCRJob.extracted_text.isnot(None)).scalar() or 0
    )
    ...
    return {
        ...
        "features": ["DOCUMENT_TEXT_DETECTION", "TEXT_DETECTION", "HANDWRITING_RECOGNITION"],
    }
