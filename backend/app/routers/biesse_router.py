...
OptiPlan 360 ile Biesse OptiPlanning sistemleri arasındaki entegrasyon endpoint'leri
...
class MaterialExportRequest(BaseModel):
    ...
    include_drops: bool = Field(True, description="Artık malzemeler dahil edilsin")


class CuttingJobRequest(BaseModel):

    job_name: Optional[str] = Field(None, description="İş adı")
...
async def get_biesse_status(
    ...
    try:
        ...
        return BiesseStatusResponse(
            ...
            message="Biesse durumu başarıyla getirildi",
        )
...
async def import_cutting_plan_from_biesse(
    ...
    try:
        ...
        if current_user.role not in ["ADMIN", "OPERATOR"]:
            ...
            raise BusinessRuleError("Kesim planı import için yetkiniz yok")
...
async def create_cutting_job(
    ...
    Sipariş kalemleri için Biesse kesim işi oluşturur
    ...
    try:
        ...
        if current_user.role not in ["ADMIN", "OPERATOR"]:
            ...
            raise BusinessRuleError("Kesim işi oluşturma için yetkiniz yok")

        # Import format validasyonu

        if request.import_format not in ["basic", "advanced"]:

            raise ValidationError("Import format 'basic' veya 'advanced' olmalıdır")
...
async def sync_materials_bidirectional(
    ...
    İki yönlü malzeme senkronizasyonu (background task)
    ...
    try:
        ...
        if current_user.role not in ["ADMIN"]:
            ...
            raise BusinessRuleError("İki yönlü senkronizasyon için yetkiniz yok")
        ...
        return {
            "success": True,
            "message": "İki yönlü senkronizasyon başlatıldı",
            "task_status": "running",
        }
...
async def get_biesse_jobs(
    ...
    Biesse iş dosyalarını listeler
    ...
    try:
        ...
        if current_user.role not in ["ADMIN", "OPERATOR"]:
            ...
            raise BusinessRuleError("İş listesi için yetkiniz yok")
        ...
        return {
            ...
            "message": "İş listesi başarıyla getirildi",
        }
...
async def get_biesse_materials(
    ...
    Biesse malzeme dosyalarını listeler
    ...
    try:
        ...
        return {
            ...
            "message": "Malzeme dosyaları başarıyla getirildi",
        }
...
async def test_biesse_connection(
    ...
    Biesse OptiPlanning bağlantısını test eder
    ...
    try:
        ...
        if current_user.role not in ["ADMIN", "OPERATOR"]:
            ...
            raise BusinessRuleError("Bağlantı testi için yetkiniz yok")
        ...
        return {
            ...
            "message": (
                ...
                else "Bağlantı sorunları tespit edildi"
            ),
        }
...
async def cleanup_biesse_files(
    ...
    Eski Biesse dosyalarını temizler
