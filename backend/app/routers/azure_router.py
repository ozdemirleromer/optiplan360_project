...
from app.models import AzureConfig, AuditLog, User, OCRJob
from app.auth import get_current_user
from app.services.azure_service import (
    AzureService, AzureOCRConfig, AzureBlobConfig, 
    AzureConfigOut, AzureOCRResult, AzureBlobUploadResult
)
...
async def azure_ocr_upload(
    ...
    if store_in_blob and service.is_configured()["blob"]:
        upload_result = await service.upload_to_blob(
            contents, 
            file.filename, 
            file.content_type
        )
    ...
    for idx, line_data in enumerate(ocr_result.lines, 1):
        ...
        ocr_line = OCRLine(
            ...
            line_number=idx,
            text=text,
            confidence=line_data.get("words", [{}])[0].get("confidence", 0.8) if line_data.get("words") else 0.8,
...
async def azure_blob_upload(
    ...
    result = await service.upload_to_blob(
        contents, 
        file.filename, 
        file.content_type
    )
...
def get_azure_stats(
    ...
    total_ocr = db.query(func.count(OCRJob.id)).filter(
        OCRJob.created_at >= datetime.now(timezone.utc).replace(day=1)
    ).scalar() or 0
