...
from ..models import IntegrationTypeEnum, User
...
def bulk_sync_accounts(
    ...
    try:
        ...
        results = {"total": len(request.entity_ids), "success": 0, "failed": 0, "details": []}
        ...
        for account_id in request.entity_ids:
            try:
                ...
                results["details"].append({"account_id": account_id, "result": result})
            except Exception as e:
                results["failed"] += 1
    ...
    try:
        ...
        for invoice_id in request.entity_ids:
            try:
                ...
                results["details"].append({"invoice_id": invoice_id, "result": result})
            except Exception as e:
                results["failed"] += 1
                results["details"].append({"invoice_id": invoice_id, "error": str(e)})
    ...
    try:
        ...
        for quote_id in request.entity_ids:
            try:
                ...
                results["details"].append({"quote_id": quote_id, "result": result})
            except Exception as e:
                results["failed"] += 1
                results["details"].append({"quote_id": quote_id, "error": str(e)})
    ...
    try:
        ...
        for order_id in request.entity_ids:
            try:
                ...
                results["details"].append({"order_id": order_id, "result": result})
            except Exception as e:
                results["failed"] += 1
