...
async def generate_text_response(
    ...
    try:
        ...
        if not response["success"]:
            raise BusinessRuleError(f"AI yanıt üretilemedi: {response.get('error', 'Bilinmeyen hata')}")
...
async def extract_structured_data(
    ...
    try:
        ...
        if not response["success"]:
            raise BusinessRuleError(f"Veri çıkarılamadı: {response.get('error', 'Bilinmeyen hata')}")
...
async def analyze_document(
    ...
    try:
        ...
        if not response["success"]:
            raise BusinessRuleError(f"Doküman analiz edilemedi: {response.get('error', 'Bilinmeyen hata')}")
...
async def analyze_image(
    ...
    try:
        ...
        if not response["success"]:
            raise BusinessRuleError(f"Görüntü analiz edilemedi: {response.get('error', 'Bilinmeyen hata')}")
...
async def health_check(
    ...
    try:
        ...
        test_response = await gemini_service.generate_text_response(
            prompt="Test",
            system_instruction="Kısa cevap ver"
        )
