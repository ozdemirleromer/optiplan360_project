...
from app.schemas import (
    PortalDashboardStats, PortalOrderOut, PortalInvoiceOut,
    PortalTicketOut, PortalTicketMessageOut, PortalTicketCreate, PortalTicketReply
)
import uuid
...
def get_customer_invoices(
    ...
    result = []
    for inv in invoices:
        result.append(PortalInvoiceOut(
            id=inv.id,
...
def get_customer_ticket_detail(
    ...
    ticket = db.query(CRMTicket).filter(
        CRMTicket.id == ticket_id,
        CRMTicket.account_id == current_user.crm_account_id
    ).first()
