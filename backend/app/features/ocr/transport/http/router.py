...
def _extract_phone_from_text(text: str) -> Optional[str]:
    ...
    """Metin içinden telefon numarası çıkar"""
    ...
    # Türkiye telefon formatları: 05XX XXX XX XX, +90 5XX ..., 5XX ...




    pattern = r"(?:\+?90\s*)?0?\s*(5\d{2})[\s\-]?(\d{3})[\s\-]?(\d{2})[\s\-]?(\d{2})"
...
def _match_customer_by_phone(db: Session, phone: str) -> Optional[Customer]:
    ...
    variants = {
        normalized,
        normalized.replace("+", ""),
        normalized.removeprefix("+90"),
        f"0{normalized.removeprefix('+90')}",
    }
    customer = db.query(Customer).filter(Customer.phone.in_(variants)).first()
    ...
    digits_only = normalized.removeprefix("+90")
    if len(digits_only) >= 7:

        partial = digits_only[-7:]
...
def _extract_thickness_from_text(text: str) -> str | None:
    match = re.search(r"(\d+(?:[.,]\d+)?)\s*mm", str(text or ""), flags=re.IGNORECASE)
    if not match:
        return None
...
def get_ocr_config(
    ...
    preprocessing_enabled = (
        ...
        _get_ocr_kv(db, "ocr_preprocessing_enabled") or "true"
...
def update_ocr_config(
    ...
    if config.preprocessing_enabled is not None:
        ...
        _set_ocr_kv(
            ...
            db, "ocr_preprocessing_enabled", "true" if config.preprocessing_enabled else "false"
...
async def upload_image(
    ...
    if file.content_type not in allowed_types:
        ...
        raise BusinessRuleError(
            ...
            f"Geçersiz dosya tipi: {file.content_type}. İzin verilen: {', '.join(allowed_types)}"
                    ...
                    for idx, line_data in enumerate(ocr_result.lines, 1):
                        ...
                        if line_text:
                            ...
                            avg_confidence = (
                                ...
                                sum(w.get("confidence", 0.8) for w in words) / len(words)
    ...
    for line in job.lines:
        ...
        lines_out.append(
...

            {
                ...
                "parsed_data": _deserialize_parsed_data(line.parsed_data),
            }
...
def lookup_customer_by_phone(
    ...
    if len(normalized) >= 7:
        ...
        similar_customers = (
            ...
            db.query(Customer).filter(Customer.phone.like(f"%{partial}%")).limit(5).all()
