...
from app.features.ocr.channel_telemetry import SCANNER_DEVICE_CHANNEL, attach_channel_marker, record_channel_event
...
def _require_device_key(request: Request, db: Session, attempted_at: Optional[datetime] = None):
    ...
    if not expected or provided != expected:
        ...
        record_channel_event(
            ...
            event="DEVICE_INGEST",
            status="FAILED",
            when=attempted_at or datetime.now(timezone.utc),
            error="Cihaz API key geçersiz",
        )
        db.commit()
...
async def ingest_from_device(
    ...
    _require_device_key(request, db, attempted_at)
