...
from app.models import OptiAuditEvent, User
from app.models.enums import OptiJobStateEnum
from app.permissions import Permission
from app.schemas import OptiJobCreate, OptiJobListResponse, OptiJobOut
...
def worker_status(
    ...
    from app.services.optiplan_worker_service import get_worker_status
...
def worker_reset(
    ...
    from app.services.optiplan_worker_service import reset_circuit_breaker
...
def get_receipt(
    ...
    from app.services.production_receipt_service import get_production_receipt
...
def create_receipt(
    ...
    from app.services.production_receipt_service import create_production_receipt
...
def get_job_audit(
    ...
    """Job audit olaylarini listele (kronolojik sira)."""
    svc = OrchestratorService(db)
    svc.get_job(job_id)  # 404 kontrolu
    events = (
        db.query(OptiAuditEvent)
        .filter(OptiAuditEvent.job_id == job_id)
        .order_by(OptiAuditEvent.created_at.asc())
        .all()
    )
    return [
        {
