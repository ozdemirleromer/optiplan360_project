...
function buildMenuGroups(permissions: PermissionState, badgeCounts?: BadgeCounts): MenuGroup[] {
  ...
  const operationsItems: MenuItem[] = [
    ...
    permissions.operations.orders
      ...
      ? { id: "orders", icon: <Package size={18} aria-hidden="true" />, label: "Sipariş Listesi", badge: badgeCounts?.orders }
  ...
  const productionItems: MenuItem[] = [
    ...
    permissions.operations.orders
      ...
      ? { id: "optiplan-ui", icon: <Factory size={18} aria-hidden="true" />, label: "OptiPlan 360" }

      : null,
    permissions.operations.orders
      ? { id: "ocr-pool", icon: <FileText size={18} aria-hidden="true" />, label: "OCR Havuzu" }
      : null,
    permissions.operations.orders
      ? { id: "ocr-kontrol", icon: <ClipboardCheck size={18} aria-hidden="true" />, label: "OCR Kontrol" }
      : null,
    permissions.operations.orders
      ? { id: "siparis-kontrol", icon: <Activity size={18} aria-hidden="true" />, label: "Sipariş Kontrol" }
      : null,
    permissions.operations.orders
      ? { id: "export-page", icon: <Download size={18} aria-hidden="true" />, label: "OptiPlanning" }
      : null,
    permissions.operations.orders
      ? { id: "siparis-fisi", icon: <FileText size={18} aria-hidden="true" />, label: "Sipariş Fişi" }
      : null,
    permissions.operations.crm || permissions.operations.orders
      ? { id: "teklif-fisi", icon: <FileText size={18} aria-hidden="true" />, label: "Teklif Fişi", badge: badgeCounts?.teklif }
      : null,
    permissions.operations.crm || permissions.operations.orders
      ? { id: "stok-karti", icon: <Package size={18} aria-hidden="true" />, label: "Stok Kartı" }
      : null,
    permissions.operations.crm || permissions.operations.orders
      ? { id: "cari-karti", icon: <Users size={18} aria-hidden="true" />, label: "Cari Kartı" }
      : null,
...
export const Sidebar = ({
  ...
  const { hasPermission, loading, error, permissions: rawPerms } = usePermissions();



  const permissions = useMemo<PermissionState>(() => {

    if (loading || error || rawPerms.length === 0) {
  ...
  }, [error, hasPermission, loading, rawPerms.length, userRole]);
