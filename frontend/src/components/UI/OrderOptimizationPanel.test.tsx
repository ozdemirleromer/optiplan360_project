  ...
  it("renders restored detail actions and triggers their handlers", async () => {
    ...
    render(
      <OrderOptimizationPanel
        ...
        messages={[{ tone: "success", text: "1 satır içe aktarıldı." }]}
        totalParts={2}
        estimatedArea="0.65"
    ...
    expect(screen.getByText("Müşteri ve İletişim")).toBeInTheDocument();
    expect(screen.getByText("1 satır içe aktarıldı.")).toBeInTheDocument();
    expect(screen.getByText("0.65 m2")).toBeInTheDocument();
    ...
    await user.click(screen.getByRole("button", { name: "Optimizasyona Gönder" }));
