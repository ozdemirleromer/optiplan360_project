...
function buildTypography(theme: ThemeDef) {
  ...
  const lineHeight = LINE_HEIGHT_MAP[design.typography?.lineHeight ?? "normal"] ?? LINE_HEIGHT_MAP.normal;

  const headingWeight: number = design.typography?.headingWeight ?? 700;

  const bodyWeight: number = design.typography?.bodyWeight ?? 400;
...
let primaryRgb = runtimeTheme.primaryRgb || hexToRgb(runtimeTheme.colors.primary);
...
export function setRuntimeTheme(themeOrName: ThemeDef | ThemeName): ThemeDef {
  ...
  primaryRgb = nextTheme.primaryRgb || hexToRgb(nextTheme.colors.primary);
...
export function getStatusConfig() {
  ...
  return {
    ...
    DRAFT: { label: "Taslak", color: COLORS.muted, bg: "rgba(159,176,208,0.10)", icon: "DR" },

    NEW: { label: "Yeni", color: COLORS.primary, bg: COLORS.primary, icon: "NW" },

    PENDING_APPROVAL: { label: "Onay Bekliyor", color: COLORS.warning, bg: COLORS.warning, icon: "PA" },

    HOLD: { label: "Bekletme", color: COLORS.warning, bg: COLORS.warning, icon: "HD" },

    IN_PRODUCTION: { label: "Üretimde", color: COLORS.accent, bg: COLORS.accent, icon: "PR" },

    APPROVED: { label: "Onaylı", color: COLORS.success, bg: COLORS.success, icon: "AP" },

    READY: { label: "Hazır", color: COLORS.success, bg: COLORS.success, icon: "OK" },

    DELIVERED: { label: "Teslim", color: COLORS.primary, bg: COLORS.primary, icon: "TS" },

    CANCELLED: { label: "İptal", color: COLORS.danger, bg: COLORS.danger, icon: "IP" },

    DONE: { label: "Tamamlandı", color: COLORS.muted, bg: "rgba(255,255,255,0.06)", icon: "DN" },

    COMPLETED: { label: "Tamamlandı", color: COLORS.muted, bg: "rgba(255,255,255,0.06)", icon: "DN" },
