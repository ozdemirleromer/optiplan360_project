...
export const Select = ({
  ...
  const getFirstEnabledIndex = () => filteredOptions.findIndex((opt) => !opt.disabled);
  ...
  useEffect(() => {
    ...
    const initialIndex = selectedIndex >= 0 && !filteredOptions[selectedIndex]?.disabled
      ? selectedIndex
      : firstEnabledIndex;
    setActiveOptionIndex(initialIndex);
  }, [focused, selectedIndex, filteredOptions]);

  useEffect(() => {
    if (!focused || activeOptionIndex < 0) return;
    const activeEl = document.getElementById(`${listboxId}-option-${activeOptionIndex}`);
    activeEl?.scrollIntoView({ block: "nearest" });
  }, [activeOptionIndex, focused, listboxId]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 6, ...containerStyle }}>
      ...
      <div style={{ position: "relative" }}>
        <button
          ...
          onKeyDown={(e) => {
            if (disabled) return;
            if (!focused && (e.key === "ArrowDown" || e.key === "ArrowUp" || e.key === "Enter" || e.key === " ")) {
              e.preventDefault();
              setFocused(true);
              return;
            }
            ...
            if (e.key === "End") {
              e.preventDefault();
              setActiveOptionIndex(getLastEnabledIndex());
              return;
            }
            if (e.key === "ArrowDown") {
              e.preventDefault();
              setActiveOptionIndex((current) => getNextEnabledIndex(current < 0 ? -1 : current, 1));
              return;
            }
            if (e.key === "ArrowUp") {
              e.preventDefault();
              const start = activeOptionIndex < 0 ? filteredOptions.length : activeOptionIndex;
              setActiveOptionIndex(getNextEnabledIndex(start, -1));
              return;
            }
            if ((e.key === "Enter" || e.key === " ") && activeOptionIndex >= 0) {
              e.preventDefault();
          ...
          aria-activedescendant={focused && activeOptionIndex >= 0 ? `${listboxId}-option-${activeOptionIndex}` : undefined}
          aria-describedby={error ? `${ariaDescribedBy || ""}-error` : ariaDescribedBy}
          style={{
            width: "100%",
            ...sizeStyles[size],
