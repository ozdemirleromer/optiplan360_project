...
const OptiPlanUIPage = lazy(() => import("./components/Optimization/OptiPlanUI").then(m => ({ default: m.OptiPlanUI })));
const OptiPlanOrderEntryPage = lazy(() => import("./components/Optimization/OptiPlanStrictOrderEntry").then(m => ({ default: m.OptiPlanStrictOrderEntry })));
...
function AuthenticatedApp({ authUser }: { authUser: User }) {
  ...
  useEffect(() => {
    const onMove = (event: MouseEvent) => {
      setCursorCoords(`X:${Math.round(event.clientX)} Y:${Math.round(event.clientY)}`);
    };
  ...
  return (
    ...
    <div className="app-shell">
      ...
      <StatusBar
        data={{
          xCoord: (() => { const m = cursorCoords.match(/X:(\d+)/); return m ? Number(m[1]) : 0; })(),
          yCoord: (() => { const m = cursorCoords.match(/Y:(\d+)/); return m ? Number(m[1]) : 0; })(),
