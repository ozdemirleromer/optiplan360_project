...
export default function InvoiceList({ invoices, loading, onEdit, onDelete, onRefresh }: InvoiceListProps) {
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter((inv) => {
      ...
      return (
        inv.invoiceNumber.toLowerCase().includes(normalizedSearchText) ||
        inv.accountId.toLowerCase().includes(normalizedSearchText)
      );
    });
  }, [invoices, filterStatus, normalizedSearchText]);
    ...
    <Card style={{ overflow: "hidden" }}>
      ...
      <div style={{ padding: 16, borderBottom: `1px solid ${COLORS.border}`, display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr auto", alignItems: "end" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button type="button" onClick={() => exportInvoicesToPDF(filteredInvoices)} style={{ padding: "8px 12px", background: COLORS.danger, color: "white", border: "none", borderRadius: RADIUS.md, cursor: "pointer", fontSize: TYPOGRAPHY.fontSize.xs, fontWeight: "600", display: "flex", alignItems: "center", gap: 4 }}><Download size={14} /> PDF</button>


          <button type="button" onClick={() => exportInvoicesToExcel(filteredInvoices)} style={{ padding: "8px 12px", background: COLORS.success, color: "white", border: "none", borderRadius: RADIUS.md, cursor: "pointer", fontSize: TYPOGRAPHY.fontSize.xs, fontWeight: "600", display: "flex", alignItems: "center", gap: 4 }}><Download size={14} /> Excel</button>
              ...
              <tr key={invoice.id} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <td style={{ padding: 12, textAlign: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                    ...
                    <button type="button" onClick={() => onEdit(invoice)} style={{ border: "none", background: "transparent", cursor: "pointer", color: COLORS.primary, padding: 4 }} title="Düzenle"><Edit2 size={16} /></button>


                    <button type="button" onClick={() => handleDelete(invoice)} style={{ border: "none", background: "transparent", cursor: "pointer", color: COLORS.danger, padding: 4 }} title="Sil"><Trash2 size={16} /></button>
