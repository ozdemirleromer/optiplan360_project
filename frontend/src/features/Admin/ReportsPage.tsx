...
export function ReportsPage() {
  ...
  const { completed, pending, delayed, inProduction } = useMemo(() => {
    let c = 0, p = 0, d = 0, ip = 0;
    for (const o of orders) {
      if (o.status === "DELIVERED") c++;
      else if (o.status === "HOLD") d++;
      if (o.status === "IN_PRODUCTION") ip++;
      if (o.status === "IN_PRODUCTION" || o.status === "NEW") p++;
    }
    return { completed: c, pending: p, delayed: d, inProduction: ip };
  }, [orders]);
  ...
  }), [avgDaily, totalOrders, completed, inProduction, pending, completionRate]);
