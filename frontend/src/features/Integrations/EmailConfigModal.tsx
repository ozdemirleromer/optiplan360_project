      ...
      >
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div>
            ...
            <input type="text" value={config.imapHost} onChange={(e) => handleChange("imapHost", e.target.value)} placeholder="imap.gmail.com" style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: "transparent", color: COLORS.text }} />
          </div>

          {/* IMAP Port & SSL */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: COLORS.text, marginBottom: 6 }}>Port</label>
              <input type="number" value={config.imapPort} onChange={(e) => handleChange("imapPort", parseInt(e.target.value) || 993)} placeholder="993" style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: "transparent", color: COLORS.text }} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: COLORS.text, marginBottom: 6 }}>SSL/TLS</label>
              <button type="button" onClick={() => handleChange("useSsl", !config.useSsl)} style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: config.useSsl ? `${COLORS.success}20` : "transparent", color: config.useSsl ? COLORS.success : COLORS.text, cursor: "pointer", fontWeight: 500 }}>
          ...
          <div>
            ...
            <input type="email" value={config.imapUser} onChange={(e) => handleChange("imapUser", e.target.value)} placeholder="ornek@email.com" style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: "transparent", color: COLORS.text }} />
          </div>
          ...
          <div>
            ...
            <input type="password" value={config.imapPass} onChange={(e) => handleChange("imapPass", e.target.value)} placeholder="\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: "transparent", color: COLORS.text }} />
            <p style={{ margin: "4px 0 0", fontSize: 11, color: COLORS.muted }}>Gmail i\u00E7in uygulama \u015Fifresi kullan\u0131n</p>
          </div>

          {/* Mailbox */}
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: COLORS.text, marginBottom: 6 }}>Mailbox</label>
            <input type="text" value={config.imapMailbox} onChange={(e) => handleChange("imapMailbox", e.target.value)} placeholder="INBOX" style={{ width: "100%", padding: "10px 12px", borderRadius: RADIUS.md, border: `1px solid ${COLORS.border}`, fontSize: 14, fontFamily: TYPOGRAPHY.fontFamily.base, background: "transparent", color: COLORS.text }} />
          </div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
            <Button variant="secondary" size="sm" onClick={handleTest} disabled={testing || !config.imapHost || !config.imapUser || !config.imapPass}>
              {testing ? "Test Ediliyor..." : "Ba\u011Flant\u0131 Testi"}
            </Button>
            <div style={{ flex: 1 }} />
            <Button variant="ghost" size="sm" onClick={onClose}>\u0130ptal</Button>
            <Button variant="primary" size="sm" onClick={handleSave} disabled={saving || !config.imapHost || !config.imapUser || !config.imapPass}>
