...
export function AWSConfigModal({ isOpen, onClose, onSave, initialConfig }: AWSConfigModalProps) {
  const [config, setConfig] = useState<AWSConfig>(initialConfig || defaultConfig);
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState<{ success: boolean; message: string } | null>(null);
  const [saving, setSaving] = useState(false);

  const handleChange = useCallback((field: keyof AWSConfig, value: string | boolean) => {
  ...
  const handleTest = useCallback(async () => {
    ...
    setTestResult(null);
    try {
      const result = await adminService.testAws();
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      await adminService.updateAwsConfig({
        ...
        >
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: COLORS.muted }}>
              Amazon Web Services OCR servisi ayarları
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: COLORS.muted }}>AWS Textract servisini aktif/pasif yap</div>
            </div>
            <button
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: COLORS.text, marginBottom: 6 }}>
              Access Key ID <span style={{ color: COLORS.error.DEFAULT }}>*</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: COLORS.text, marginBottom: 6 }}>
              Secret Access Key <span style={{ color: COLORS.error.DEFAULT }}>*</span>
            </label>
            <input
              type="password"
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
            ...
            <Button
              ...
              size="sm"
              onClick={handleSave}
              disabled={saving || !config.accessKeyId || !config.secretAccessKey}
            >
