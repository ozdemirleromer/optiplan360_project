...
import { DEFAULT_MATERIAL, MATERIAL_SPECIFICATIONS } from "./orderOptimizationConstants";
import { normalizeMaskedNumberOnBlur, parseMaskedNumber, sanitizeIntegerInput, sanitizeMaskedNumberInput } from "../OrderEditor/workbenchUtils";
  ...
  return (
    ...
    >
      ...
      <div className={orderOptimizationStyles.cell}>
        <select
          ref={(node) => {
            ...
            if (firstCellRef && rowIndex === 0) {
              (firstCellRef as unknown as MutableRefObject<HTMLSelectElement | null>).current = node;
            }
          }}
        ...
        >
          {MATERIAL_SPECIFICATIONS.map((spec, idx) => (
            <option key={`${spec.thickness}-${spec.dimensions}-${idx}`} value={`${spec.thickness} ${spec.dimensions}`} style={{ color: "#e0e4ea", backgroundColor: "#1e1e1e" }}>
      ...
      <div className={`${orderOptimizationStyles.cell} justify-center`}>
        ...
        >
          <option value="0" style={{ color: "#e0e4ea", backgroundColor: "#1e1e1e" }}>0</option>
          <option value="1" style={{ color: "#e0e4ea", backgroundColor: "#1e1e1e" }}>1</option>
          <option value="2" style={{ color: "#e0e4ea", backgroundColor: "#1e1e1e" }}>2</option>
          <option value="3" style={{ color: "#e0e4ea", backgroundColor: "#1e1e1e" }}>3</option>
        </select>
      </div>
...
export function OrderOptimizationGrid({ items, plateBoy, notice: _notice, firstCellRef, onItemChange, onAddItem: _onAddItem, onRemoveItem, onDuplicateItem }: OrderOptimizationGridProps) {
