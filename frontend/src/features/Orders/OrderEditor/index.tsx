...
import { OrderOptimizationPanel } from "../OrderOptimization/OrderOptimizationPanel";
import type { OrderOptimizationRibbonTab } from "../OrderOptimization/OrderOptimizationRibbon";
...
export function OrderEditor({ order, onBack }: OrderEditorProps) {
  const [, setTab] = useState<"measures" | "validate" | "export">("measures");
  ...
  }, [order?.id, order?.mat, order?.status, order?.thick, partsLoaded]);
  ...
  const mapRowsToBackendParts = useCallback((sourceRows: MeasureRow[]): BackendOrderPart[] => {
  ...
  const resolveCustomerId = useCallback(async (phoneNorm: string): Promise<number> => {
  ...
  const saveOrderToDB = useCallback(async () => {
    ...
    return newId;
  }, [customerPhone, mapRowsToBackendParts, material, order?.id, plateSize, priority, resolveCustomerId, rows, thickness]);
  ...
  }, [rows, customerName, runValidation, isSaving, saveOrderToDB]);
  ...
  const handleImportFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (extension === "xlsx" || extension === "xls") {
        ...
        const dataRows = XLSX.utils.sheet_to_json<unknown[]>(worksheet, { header: 1 });
